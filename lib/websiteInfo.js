...
### 4. Video Toolkit (Path: /tools/video)
  - **Video Toolkit (/tools/video):** Trims, edits, and converts local video files (MP4, WebM, etc.) client-side via a WebAssembly FFmpeg instance, allowing users to export video clips as animated GIFs with custom FPS, scale, and trim ranges.
...
### 7. DinoShare (Path: /tools/dinoshare)
  - A secure cross-device file & text sharing toolkit consisting of two sub-tools:
    - **File Share (/tools/dinoshare/file):** Users can upload files of any size, receive a 4-digit code + QR code, and download the file from any other device. Uses direct-to-storage upload/download via Supabase Storage signed URLs.
    - **Text Clipboard (/tools/dinoshare/text):** Users can paste text clips, receive a 4-digit code + QR code, and view/copy the text from any other device instantly.
  - All shared items automatically expire after 24 hours.
